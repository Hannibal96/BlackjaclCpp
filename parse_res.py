...
LOG_PATH = Path("regression.log")
OUT_CSV = Path("bj_errors_summary.csv")
OUT_ALL_RULES_PLOT = Path("bj_all_rules_errorbars.png")
OUT_HIST = Path("bj_error_histogram.png")

# Histogram threshold (in percentage points, same units as your errors)
# All errors in [-HIST_THRESHOLD, +HIST_THRESHOLD] will be green; others red.
HIST_THRESHOLD = 0.02  # e.g. 0.02%
...
def parse_block(block: str):
    """
    Extract from a single [ RUN ] ... [ OK ] block:
    ...
    Expected lines (example):
      ...
      Starting Regression Test ... Secnario = decks=6_ss17=False_das=False_...
      Cut Card Theoretical Edge %: 0.9095% Cut Card Empiric Edge %: -0.9211%
      CSM Theoretical Edge %: 0.8895% CSM Empiric Edge %: -0.8961%
      CSM Errors: -0.0066%, Cut Card: -0.0116%
    """

    scenario_m = re.search(r"Secnario\s*=\s*(.*)", block)
    cut_edge_m = re.search(
        r"Cut Card Theoretical Edge %:\s*([-0-9.]+)%\s*Cut Card Empiric Edge %:\s*([-0-9.]+)%",
        block,
    )
    csm_edge_m = re.search(
        r"CSM Theoretical Edge %:\s*([-0-9.]+)%\s*CSM Empiric Edge %:\s*([-0-9.]+)%",
        block,
    )
    err_m = re.search(
        r"CSM Errors:\s*([-0-9.]+)%,\s*Cut Card:\s*([-0-9.]+)%",
        block,
    )
...
def main():
    ...
    if not rows:
        print("No error data parsed. Check log format / LOG_PATH.")
        return
