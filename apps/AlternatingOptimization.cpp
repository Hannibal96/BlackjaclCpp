...
struct AlternatingOptimizationApp {
    using Rules = typename Game::Rules;
    using Case  = typename Game::Case;


    static constexpr std::array<double, 13> ZERO_WEIGHTS = {};
    static constexpr double kEps = 1e-12;
    static inline const std::string kAlternatingCheckpointRoot =
        std::string("checkpoints/") + Game::kCheckpointDirPrefix + "alternating-checkpoints";
    static constexpr double kTargetTenValueTag = -1.0;
    static constexpr double kKellyInitialMoney = 1.0;
    static constexpr double kKellyFraction = 1.0;
    static constexpr uint64_t kKellyMeasurementRounds = 1'000'000ULL;
    ...
    static inline uint64_t    g_sample_every        = 1;
    static inline uint64_t    g_sample_rounds       = 100'000'000ULL;
    static inline double      g_diff_threshold      = 0.001;
    static inline TrainingStopMode g_training_stop_mode = TrainingStopMode::FIXED_ROUNDS;
    static inline CountRegressionObjective g_count_regression_objective =
    ...
    static inline bool        g_verbose             = false;
    static inline bool        g_full_verbose        = false;
    static inline bool        g_no_save             = false;
    static inline std::string g_checkpoint_name;
    static inline std::string g_load_checkpoint;
    ...
    static inline AgentConfig g_agent;

    static const char* countRegressionObjectiveToString(CountRegressionObjective objective) {
        switch (objective) {
    ...
    static const char* countRegressionConstraintToString(CountRegressionConstraint constraint) {
        switch (constraint) {
    ...
    static CountRegressionObjective stringToCountRegressionObjective(const std::string& value) {
        return value == "quadratic_kelly"
            ? CountRegressionObjective::QUADRATIC_KELLY
            : CountRegressionObjective::CLASSICAL_OLS;
    }

    static CountRegressionConstraint stringToCountRegressionConstraint(const std::string& value) {
    ...
    static std::filesystem::path runDirectory(const std::string& folder) {
        if (g_output_dir.empty())
            return std::filesystem::path(PROJECT_ROOT) / kAlternatingCheckpointRoot / folder;
        std::filesystem::path path(g_output_dir);
        return path.is_absolute() ? path : std::filesystem::path(PROJECT_ROOT) / path;
    }

    static std::string ToString(const Case& c) { return Game::toString(c); }
    static std::string ToStringTableName(const Case& c) { return Game::toStringTableName(c); }

    // AlternatingOptimization checkpoint folders key on Game::checkpointFolderName
    // rather than ToStringTableName so multi-deck research runs never collide
    // (see GameTraits.h for why the two can differ for Blackjack).
    static std::string checkpointFolderName(const Case& c) { return Game::checkpointFolderName(c); }

    static std::vector<Case> generateCases() {
        return Game::generateCases(g_deck_sizes, g_stand_soft17);
    }
    ...
    static std::string buildRunHeader(const Case& c, const std::string& folder,
        ...
        os << "Folder:        " << runDirectory(folder).string() << "/\n";
    ...
    static json buildMetaJson(const Case& c) {
        json meta;
        meta["algorithm"] = std::string(Game::kAlgorithmPrefix) + "alternating_optimization";
        meta["artifacts"]["naming"] = "W0 (implicit zeros), P0, W1, P1, W2, P2, ...";

        meta["game"]["decks"]       = c.deckSize;
        meta["game"]["ss17"]        = c.standSoft17;
        Game::writeMetaGameSection(meta, c);
    ...
    static Rules buildRules(const Case& c, double minBet = 1.0, double maxBet = 1.0) {
        return Game::buildRules(c, g_penetration, minBet, maxBet);
    }

    static std::unique_ptr<QLearningStrategy> makeQStrategy() {
    ...
    static double averageRoundsPerThread(uint64_t rounds, int threads) {
        return static_cast<double>(rounds) / static_cast<double>(threads);
    }

    static double netPerRoundFromPlayer(const Player& player, uint64_t rounds, int threads) {
    ...
    static uint64_t countPhasePlayedRounds() {
        if (g_sample_every == 0)
    ...
    static EdgeStatistics evaluateEdge(const Case& c,
        ...
        Rules rules = buildRules(c, minBet, maxBet);
    ...
    static KellyEvaluationResult evaluateKellyGrowth(const Case& c,
                                              const BasicStrategy& strategy,
                                              const CountConfig& count,
                                              uint64_t rounds,
    ...
    static std::array<double, 14> solveUnconstrainedNormalEquation(
    ...
    static std::array<double, 14> solveSumZeroNormalEquation(
            const std::array<std::array<double, 14>, 14>& A,
    ...
    static std::array<double, 14> solveSumZeroFixedBiasNormalEquation(
    ...
    static double spreadThresholdFromCount(const CountConfig& count) {
    ...
    static json countConfigToJson(const CountConfig& count, bool roundWeights = true) {
        json j;
    ...
    static json evCountGraphToJson(const EvCountGraphArtifact& graph) {
        json j;
    ...
    static json countHistogramToJson(const EvCountGraphArtifact& graph) {
        json j;
        j["resolution"] = graph.resolution;
        j["min_count"] = graph.minCount;
        j["max_count"] = graph.maxCount;
    ...
    static EvCountGraphArtifact evCountGraphFromJson(const json& j) {
        EvCountGraphArtifact graph;
    ...
    static std::string evCountGraphsToJson(const std::vector<NamedEvCountGraphArtifact>& graphs) {
        json j;
        j["graphs"] = json::array();
    ...
    static std::string colorForGraph(size_t index) {
    ...
    static ConditionalSecondMomentCurve conditionalSecondMomentCurve(
    ...
    static std::string evCountGraphToSvg(const std::string& label,
                                  const EvCountGraphArtifact& graph) {
    ...
    static std::string evCountOverlaySvg(const std::string& label,
    ...
    static std::string countHistogramToSvg(const std::string& label,
                                    const EvCountGraphArtifact& graph) {
        const double width = 1000.0;
    ...
    static CountConfig countConfigFromJson(const json& j) {
        CountConfig count;
    ...
    static CountConfig makeZeroCount() {
        CountConfig count;
    ...
    static Phase stringToPhase(const std::string& phase) {
        if (phase == "count") return Phase::COUNT;
        if (phase == "done") return Phase::DONE;
        return Phase::POLICY;
    }

    static void saveMeta(const std::string& folder, const json& meta) {
        namespace fs = std::filesystem;
        fs::create_directories(runDirectory(folder));
        std::ofstream f(runDirectory(folder) / "meta.json");
        if (f.is_open()) f << meta.dump(2);
    }

    static void saveState(const std::string& folder, const ResumeState& state) {
        namespace fs = std::filesystem;
        fs::path root = runDirectory(folder);
    ...
    static ResumeState inferResumeState(const std::string& folder) {
        namespace fs = std::filesystem;
        fs::path root = runDirectory(folder);
        ...
        if (maxP >= 0 && maxW < maxP + 1) {
            ...
            state.currentPolicyPath =
                (root / (state.currentPolicyLabel + "_strategy.json")).string();
    ...
    static ResumeState loadState(const std::string& folder) {
        namespace fs = std::filesystem;
        fs::path path = runDirectory(folder) / "state.json";
        if (!fs::exists(path)) {
    ...
    static Case loadCheckpointFolder(const std::string& folder) {
        namespace fs = std::filesystem;
        fs::path metaPath = runDirectory(folder) / "meta.json";
        ...
        auto& g = meta.at("game");
        Case c = Game::readMetaGameSection(g);
        ...
        if (a.contains("count_regression_objective")) {
            ...
            if (legacyMode == "quadratic_kelly") {
                g_count_regression_objective = CountRegressionObjective::QUADRATIC_KELLY;
                g_count_regression_constraint = a.value("quadratic_kelly_zero_bias", false)
                    ? CountRegressionConstraint::FIXED_ZERO_BIAS
                    : CountRegressionConstraint::NONE;
            } else {
    ...
    static void savePolicyArtifact(const std::string& folder,
                            const std::string& label,
        ...
        artifact.qStrategy->saveToFile(
            (root / (label + "_agent.json")).string(), artifact.trainingRounds);
        artifact.policy->saveToJson((root / (label + "_strategy.json")).string());
    ...
    static void saveCountArtifact(const std::string& folder,
                           const std::string& label,
    ...
    static void saveEvCountGraphArtifact(const std::filesystem::path& root,
                                  const std::string& label,
                                  const EvCountGraphArtifact& graph) {
        namespace fs = std::filesystem;
        fs::create_directories(root);
    ...
    static void saveCumulativeEvCountGraphArtifact(const std::filesystem::path& root,
                                            int upToWeightIndex) {
        namespace fs = std::filesystem;
    ...
    static void saveKellyGrowthGraphArtifact(const std::filesystem::path& root,
                                      const std::string& label,
    ...
    static PolicyArtifact learnPolicy(const Case& c,
                               const CountConfig& count,
                               uint64_t rounds,
                               const std::string& label,
    ...
    static CountArtifact learnCount(const Case& c,
                             const std::string& label,
    ...
    static EvCountGraphArtifact measureEvCountGraph(const Case& c,
                                             const BasicStrategy& fixedPolicy,
                                             const CountConfig& count,
                                             uint64_t rounds) {
        auto cloned = fixedPolicy.clone();
    ...
    static void printCountSummary(const std::string& label, const CountArtifact& artifact) {
        ...
        if (artifact.regressionConstraint == CountRegressionConstraint::FIXED_ZERO_BIAS) {
            std::cout << "fixed to zero\n";
        } else if (!artifact.forcedBias.has_value()) {
            std::cout << "free\n";
    ...
    static void printEvCountGraphSummary(const std::string& label, const EvCountGraphArtifact& graph) {
    ...
    static void printPolicySummary(const std::string& label, const PolicyArtifact& artifact) {
        const uint64_t evalRounds = (g_eval_rounds == 0 ? g_num_rounds : g_eval_rounds);
        std::cout << "\n--- " << label << " ---\n";
    ...
    static void runCase(const Case& c) {
        ...
        while (state.nextPhase != Phase::DONE) {
            if (state.nextPhase == Phase::POLICY) {
                ...
                state.currentPolicyPath =
                    (runDirectory(folder) / (pLabel + "_strategy.json")).string();
    ...
    static void printHelp(const char* prog) {
        ...
        std::cout << "Alternating optimization between policy learning (RL) and configurable\n";
        std::cout << "count-regression objectives.\n\n";
        ...
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/meta.json      Run config\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/state.json     Resume state\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/P*.json        Policy summaries\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/P*_agent.json  Learned Q-tables\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/P*_strategy.json  Greedy policy tables\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/W*.json               Count summaries\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/W*_data.json          Regression matrices\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/W*_graph.json/svg     Single EV-vs-count graph\n";
        std::cout << "    " << kAlternatingCheckpointRoot << "/<folder>/W*_second_moment_graph.json/svg\n";
        ...
        std::cout << "  --checkpoint-name <name>  Folder prefix under " << kAlternatingCheckpointRoot << "/ (default: timestamp)\n";
        std::cout << "  --load-checkpoint <name>  Resume from " << kAlternatingCheckpointRoot << "/<name>/\n";
        std::cout << "  --output-dir <path>     Write this run directly to path instead of the default checkpoint root\n";
        ...
        std::cout << "  --count-quadratic-kelly-zero-bias\n";
        std::cout << "                                Legacy alias for quadratic Kelly with bias fixed to zero.\n";
        ...
        std::cout << "  --decks <list>            Shoe sizes (default: 6)\n";
        std::cout << "  --ss17 <list>             Dealer stands on soft 17 (default: "
                  << (Game::kDefaultStandSoft17 ? "true" : "false/H17") << ")\n";
        Game::printGameHelp();
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--count-quadratic-kelly-zero-bias") {
            g_count_regression_objective = CountRegressionObjective::QUADRATIC_KELLY;
            g_count_regression_constraint = CountRegressionConstraint::FIXED_ZERO_BIAS;
            ++countObjectiveFlags;
...
int runBlackjackAlternatingOptimization(int argc, char** argv) {
    return AlternatingOptimizationApp<BlackjackGame>::run(argc, argv);
}
