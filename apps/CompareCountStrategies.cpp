...
struct CompareCountStrategiesApp {
    using Rules = typename Game::Rules;
    using Case  = typename Game::Case;

    // --deviations-checkpoint reads AlternatingOptimization's own Pk/Wk
    // artifacts (see GameTraits.h) so each game's checkpoint root matches
    // whatever AlternatingOptimizationApp<Game> wrote.
    static inline const std::string kQlearningCheckpointRoot =
        std::string("checkpoints/") + Game::kCheckpointDirPrefix + "alternating-checkpoints";
    static constexpr const char* kOlsCheckpointRoot = "checkpoints/checkpoints_ols";
    static inline const std::string kCompareCheckpointRoot =
        std::string("checkpoints/") + Game::kComparePrefix + "CompareCountStrategies";
    static constexpr uint64_t kKellyMeasurementRounds = 1'000'000ULL;
    static inline double g_referenceBias = -0.005;
    static inline double g_referenceFactor = 0.005;
    ...
    static inline int         g_min_count = -5;
    static inline int         g_max_count = 5;
    ...
    static inline std::string g_count_ols;
    static inline double g_factor = std::numeric_limits<double>::quiet_NaN();
    static inline double g_bias   = std::numeric_limits<double>::quiet_NaN();

    static inline std::string g_deviations_checkpoint;
    static inline std::string g_deviations_agent;
    static inline std::string g_strategy_checkpoint;
    ...
    static bool countWasExplicitlyConfigured() {
    ...
    static StrategyCountConfig resolveCountConfig() {
    ...
    static Rules buildRules(const Case& c, double minBet, double maxBet) {
        return Game::buildRules(c, g_penetration, minBet, maxBet);
    }

    static json graphsToJson(const std::vector<StrategyGraphArtifact>& graphs) {
        json root;
    ...
    static std::string graphsToSvg(const std::vector<StrategyGraphArtifact>& graphs) {
    ...
    static json histogramToJson(const StrategyGraphArtifact& graph) {
        json root;
    ...
    static std::string histogramToSvg(const StrategyGraphArtifact& graph) {
        const double width = 1200.0;
    ...
    static EdgeStatistics evaluateSpreadEdge(const Case& c,
                                      const BasicStrategy& strategy,
                                      const StrategyCountConfig& count,
                                      uint64_t rounds) {
        ...
        Rules rules = buildRules(c, 1.0, 10.0);
        std::vector<Player*> results = runParallelSimulation(rules, {player}, rounds, g_num_threads);
        delete player;
        if (results.empty())
    ...
    static KellyEvaluationResult evaluateKellyGrowth(const std::string& label,
                                              const Case& c,
                                              const BasicStrategy& strategy,
                                              const StrategyCountConfig& count,
    ...
    static std::unique_ptr<BasicStrategy> loadBasicStrategyForCase(const Case& c) {
        return Game::loadBasicStrategy(c);
    }

    // Illustrious 18 reference comparison is Blackjack-only (see
    // Game::kSupportsIllustrious18); these helpers are only ever called under
    // that compile-time gate, but stay always-compiled since they don't
    // reference anything game-specific themselves.
    static bool isHiLoWeights(const std::array<double, 13>& weights) {
    ...
    static std::unique_ptr<BasicStrategy> expandBasicStrategyAcrossCounts(const BasicStrategy& base,
                                                                          int minCount,
                                                                          int maxCount) {
    ...
    static void applyStandardIllustrious18(BasicStrategy& strategy,
                                           int minCount,
                                           int maxCount) {
    ...
    static void printIllustrious18List() {
    ...
    static std::unique_ptr<BasicStrategy> trainFullDeviations(const Case& c,
                                                       const StrategyCountConfig& count) {
    ...
    static std::unique_ptr<BasicStrategy> loadFullDeviationsFromCheckpoint(
        ...
        if (meta.contains("game")) {
            ...
            Game::appendMetaMismatch(g, c, mismatches);
            if (!mismatches.empty()) {
    ...
    // "Full deviations" strategy checkpoints owned by CompareCountStrategies
    // itself (as opposed to --deviations-checkpoint, which reads
    // AlternatingOptimization's Pk/Wk artifacts). Every fresh training run
    // saves one into its own run folder so a later run can reuse it directly
    // via --strategy-checkpoint instead of retraining under the same count.
    static std::filesystem::path resolveStrategyCheckpointRoot(const std::string& dir) {
        namespace fs = std::filesystem;
        fs::path root(dir);
        if (root.is_relative())
            root = fs::path(PROJECT_ROOT) / kCompareCheckpointRoot / dir;
        return root;
    }

    static void saveStrategyCheckpoint(const std::filesystem::path& root,
                                       const BasicStrategy& strategy,
                                       const StrategyCountConfig& count,
                                       const Case& c) {
        strategy.saveToJson((root / "full_deviations_strategy.json").string());

        json meta;
        json weights = json::array();
        for (double w : count.system.weights) weights.push_back(w);
        meta["count"]["weights"] = weights;
        ...
        std::ofstream(root / "full_deviations_meta.json") << std::setw(2) << meta << "\n";
    }

    static std::unique_ptr<BasicStrategy> loadStrategyCheckpoint(
        const std::string& dir,
        const Case& c,
        CheckpointCountConfig& outCount) {
        namespace fs = std::filesystem;
        const fs::path root = resolveStrategyCheckpointRoot(dir);
        const fs::path metaPath = root / "full_deviations_meta.json";
        if (!fs::exists(metaPath))
            throw std::runtime_error(
                "Strategy checkpoint '" + dir + "' not found or missing full_deviations_meta.json");

        json meta; { std::ifstream f(metaPath); f >> meta; }
        const auto& countJson = meta.at("count");
        for (int i = 0; i < 13; ++i)
            outCount.weights[i] = countJson.at("weights")[i].get<double>();
        outCount.factor = countJson.at("factor").get<double>();
        outCount.bias = countJson.at("bias").get<double>();
        ...
        auto strategy = std::make_unique<BasicStrategy>();
        const std::string strategyPath = (root / "full_deviations_strategy.json").string();
        if (!strategy->loadFromFile(strategyPath))
            throw std::runtime_error("Failed to load strategy checkpoint file: " + strategyPath);
        ...
        if (meta.contains("game")) {
            ...
            Game::appendMetaMismatch(g, c, mismatches);
            if (!mismatches.empty()) {
                std::cout << "WARNING: simulation rules differ from strategy checkpoint:\n";
                for (const auto& m : mismatches) std::cout << "  " << m << "\n";
            }
        }
    ...
    static EvalResult evaluateCase(const std::string& label,
    ...
    static void printResultsTable(const std::vector<EvalResult>& results) {
        std::cout << "\n=== Comparison ===\n";
    ...
    static void runCase(const Case& c) {
        ...
        if (!g_deviations_checkpoint.empty()) {
        ...
        } else if (!g_strategy_checkpoint.empty()) {
            CheckpointCountConfig ckptCount;
            fullDeviations = loadStrategyCheckpoint(g_strategy_checkpoint, c, ckptCount);
            fullNote = "Loaded from strategy checkpoint " + g_strategy_checkpoint;
            ...
            if (!countWasExplicitlyConfigured()) {
                ...
                g_referenceBias = requestedCount.system.bias;
                g_referenceFactor = requestedCount.system.factor;
                std::cout << "Count:       loaded from strategy checkpoint " << g_strategy_checkpoint << "\n";
            }
            ...
            if (countMismatch) {
                std::cout << "WARNING: loaded strategy checkpoint was trained with a different count setup.\n";
                std::cout << "         Full deviations will be evaluated on the explicitly requested count config.\n";
                fullNote += " (evaluated on requested count config; checkpoint training count differed)";
            }
        }
        ...
        if constexpr (Game::kSupportsIllustrious18) printIllustrious18List();
        ...
        if constexpr (Game::kSupportsIllustrious18) {
            if (isHiLoWeights(requestedCount.system.weights)) {
                auto i18Strategy = expandBasicStrategyAcrossCounts(
                    *basicStrategy, requestedCount.minCount, requestedCount.maxCount);
                applyStandardIllustrious18(*i18Strategy, requestedCount.minCount, requestedCount.maxCount);
                std::string note;
                if (!c.standSoft17) {
                    note = "Using standard Hi-Lo multi-deck S17 reference indices; "
        ...
        bool savedFreshStrategyCheckpoint = false;
        if (!fullDeviations) {
            ...
            saveStrategyCheckpoint(outputPath, *fullDeviations, requestedCount, c);
            savedFreshStrategyCheckpoint = true;
        }
        ...
        if (savedFreshStrategyCheckpoint) {
            ...
            std::cout << "  " << logger.pathFor("full_deviations_meta.json").string() << "\n";
        }
    }
    ...
    static void printHelp(const char* prog) {
        ...
        if constexpr (Game::kSupportsIllustrious18) {
            ...
            std::cout << "  3. Full deviations learned for the count (or loaded from an alternating checkpoint).\n\n";
        ...
        std::cout << "  --deviations-checkpoint <dir>  Load full deviations from " << kQlearningCheckpointRoot << "/<dir>/\n";
        ...
        std::cout << "  --strategy-checkpoint <dir>    Reload full deviations previously trained fresh by this\n";
        std::cout << "                                 app (and their count) from a prior run's own output\n";
        std::cout << "                                 folder, instead of --deviations-checkpoint or retraining.\n";
        std::cout << "                                 <dir> is a folder name under " << kCompareCheckpointRoot << "/\n";
        std::cout << "                                 or an absolute path to any prior run's folder.\n";
        std::cout << "  If neither checkpoint flag is given, full deviations are trained fresh using the RL\n";
        std::cout << "  flags below, and automatically saved into this run's own output folder as\n";
        std::cout << "  full_deviations_strategy.json + full_deviations_meta.json for later reuse.\n\n";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--strategy-checkpoint"   && i+1<argc) g_strategy_checkpoint = argv[++i];
    ...
    if (!g_deviations_checkpoint.empty() && !g_strategy_checkpoint.empty()) {
        std::cerr << "Error: choose only one of --deviations-checkpoint or --strategy-checkpoint.\n";
        return 1;
    }
...
int runBlackjackCompareCountStrategies(int argc, char** argv) {
    return CompareCountStrategiesApp<BlackjackGame>::run(argc, argv);
}
