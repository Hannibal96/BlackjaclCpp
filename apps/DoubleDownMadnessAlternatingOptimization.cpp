...
constexpr const char* kAlternatingCheckpointRoot = "checkpoints/double-down-madness-alternating-checkpoints";
...
std::filesystem::path runDirectory(const std::string& folder) {
...
std::string buildRunHeader(const MadnessCase& c, const std::string& folder, uint64_t evalRounds) {
    std::ostringstream os;
    os << "\n=== DoubleDownMadnessAlternatingOptimization ===\n";
...
json buildMetaJson(const MadnessCase& c) {
    json meta;
    meta["algorithm"] = "double_down_madness_alternating_optimization";
...
DoubleDownMadnessRules buildRules(const MadnessCase& c, double minBet = 1.0, double maxBet = 1.0) {
...
EdgeStatistics evaluateEdge(const MadnessCase& c,
    ...
    DoubleDownMadnessRules rules = buildRules(c, minBet, maxBet);
...
KellyEvaluationResult evaluateKellyGrowth(const MadnessCase& c,
                                          const BasicStrategy& strategy,
                                          const CountConfig& count,
                                          uint64_t rounds,
...
MadnessCase loadCheckpointFolder(const std::string& folder) {
    namespace fs = std::filesystem;
...
PolicyArtifact learnPolicy(const MadnessCase& c,
                           const CountConfig& count,
                           uint64_t rounds,
                           const std::string& label,
...
CountArtifact learnCount(const MadnessCase& c,
                         const std::string& label,
...
EvCountGraphArtifact measureEvCountGraph(const MadnessCase& c,
                                         const BasicStrategy& fixedPolicy,
                                         const CountConfig& count,
                                         uint64_t rounds) {
    auto cloned = fixedPolicy.clone();
...
void printHelp(const char* prog) {
    ...
    std::cout << "Double Down Madness alternating optimization between policy learning (RL)\n";
    std::cout << "and configurable count-regression objectives.\n\n";
    ...
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/meta.json      Run config\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/state.json     Resume state\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/P*.json        Policy summaries\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/P*_agent.json  Learned Q-tables\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/P*_strategy.json  Greedy policy tables\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*.json               Count summaries\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_data.json          Regression matrices\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_graph.json/svg     Single EV-vs-count graph\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_second_moment_graph.json/svg\n";
    std::cout << "                                                                      Unit-wager E[X^2 | count]\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_graph_overlay.*    Cumulative W1..Wk comparison graph\n\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_kelly_graph.*     Kelly multiplier sweep\n";
    std::cout << "    checkpoints/double-down-madness-alternating-checkpoints/<folder>/W*_kelly_graph_overlay.*  Cumulative Kelly comparison\n\n";
    ...
    std::cout << "  --checkpoint-name <name>  Folder prefix under checkpoints/double-down-madness-alternating-checkpoints/ (default: timestamp)\n";
    std::cout << "  --load-checkpoint <name>  Resume from checkpoints/double-down-madness-alternating-checkpoints/<name>/\n";
